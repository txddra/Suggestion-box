...
module.exports = {
    getAllSuggestions: (req, res) => {
        Suggestion.find().then((foundSuggestions) => {
            return res.render('main/index', {
                suggestionList: foundSuggestions
            })
        }).catch((err) => {
            res.json({
                conformation: 'fail',
    ...
    getSingleSuggestion: (req, res) => {
        ...
        .then((suggestions) => res.render('main/singleSuggestion',{
            suggestionList:foundSuggestion
        }).json({suggestions})
        .catch((err) => res.status(400).json({confirmation: 'fail',err})))
    },
    createSuggestion: (req, res) => {
        Suggestion.findById(req.params.id).then((foundSuggestion)=>{
            if(!req.body.suggestion){
                return res.status(400).json({confirmation:'fail', message:'fail'})
            }
            const newSuggestion = new Suggestion({

                givenSuggestion: req.body.suggestion,
                name: foundSuggestion.name
            });
            newSuggestion.save().then(()=>{
                return res.redirect(`/suggestions/single-suggestion/${foundSuggestion.id}`)
            }).catch((err)=>{
                return res.status(400).send('Suggestion was not saved').send
            })
        }).catch((err)=>{
        ...
        //         return res.render('main/createSuggestion', {
        //             suggestionList: foundSuggestion
        //         });
        //     }
        //     const newSuggestion = new Suggestion();
        ...
        //     newSuggestion
        //         .save()
        //         .then((foundSuggestion) => res.status(200).json({
    ...
    updateSuggestion: (req, res) => {
        Suggestion.find(req.params.title).then((foundSuggestion) => {
            ...
            foundSuggestion.save().then((suggestions) => {
                return res.status(200).json({
                    confirmation: 'Suggestion updated',
                    suggestions
                })
            })
